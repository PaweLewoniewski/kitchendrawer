...
const CornersTopView = ({ data, index, positionX, positionY }: CornersViewProp) => {
    ...
    const handleStop = (event: any, dragElement: { x: SetStateAction<number>; y: SetStateAction<number>; }) => {
        ...
        if (data !== undefined) {
            const updateData = [{ 'cornersTop': { 'id': data.id , 'cabinWidth': data.cabinWidth, 'cabinDepth': data.cabinDepth, 'name': 'cornersTop', 'xAxis': dragElement.x, 'yAxis': dragElement.y, 'side': data.side } }];
